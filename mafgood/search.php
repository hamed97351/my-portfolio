...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['query'])) {
    ...
    $tables = [];
    $excludedTables = ['users', 'contact_us']; // الجداول المستبعدة
    ...
    foreach ($tables as $table) {
        if (in_array($table, $excludedTables)) {
            continue; // تجاوز الجدول المستبعد
        }
    ...
    // تحويل النتائج إلى JSON وإرسالها إلى ملف HTML
    $jsonResults = json_encode($results, JSON_UNESCAPED_UNICODE);
    echo "<script>
